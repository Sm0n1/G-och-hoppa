...
void badCode(){
	...
	try {
		...
		WindowOptions wOptions{ "Window", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, tilesize * scale * width, tilesize * scale * height, 0 };
		RendererOptions rOptions{ -1, SDL_RENDERER_ACCELERATED };

		Graphics graphics{ wOptions, rOptions };

		auto player = registry.create();
		registry.emplace<CAction>(player);
		registry.emplace<CState>(player);
		registry.emplace<CVisual>(player, SDL_Rect{ 8 * 4, 8 * 7 - 2, 8, 10 }, SDL_Rect{ 0, 0, 32, 40 }, graphics.loadTexture("assets/Texture.png"), 1);
		registry.emplace<CSpatial>(player, SDL_Rect{ 100, 300, 16, 32 }, true);
		registry.emplace<CVelocity>(player);

		loadTilemap(registry, graphics, "assets/Tilemap.txt", "assets/Texture.png");

		InputSystem inputSystem{ engine };
		ActionSystem actionSystem{ engine };
		MoveSystem moveSystem{ engine };
		RenderSystem renderSystem{ engine };

		registry.sort<CVisual>([](const auto& vA, const auto& vB) { return vA.layer < vB.layer; });

		bool quit{ false };
		while (!quit)
		{
			clock();
			inputSystem(registry, quit);
			actionSystem(registry);
			moveSystem(registry, clock);
			renderSystem(registry, graphics);
		}
	}
	catch (...)
	{
	...
	*/
	...
	/*
	void loadTilemap(entt::registry & registry, Graphics & graphics, const std::string & tilemapFilename, const std::string textureFilename)
	{
		std::ifstream inFile(tilemapFilename);
		...
		if (inFile.is_open())
		{
			std::cout << "Tilemap file loaded successfully...\n";
		}
		else
		{
			std::cerr << "Tilemap file failed to load...\n";
			throw std::runtime_error("Error loading tilemap");
		}

		// Read file and store tilemap measurements
		...
		// Create tilemap from data one row at a time
		...
		for (int i{ 0 }; i < height; ++i)
		{
			for (int j{ 0 }; j < width; ++j)
			{
				...
				if (inFile.eof())
				{
					std::cerr << "reached end of file prematurely when loading tilemap...(" << tilemapFilename << ")\n";
					throw std::runtime_error("Tilemap: premature eof");
				}
				...
				SDL_Rect srcRect{ currentX * tileSize, currentY * tileSize, tileSize, tileSize };
				SDL_Rect dstRect{ j * tileSize * scale, i * tileSize * scale, tileSize * scale, tileSize * scale };
				SDL_Rect collisionRect{ j * tileSize * scale, i * tileSize * scale, tileSize * scale, tileSize * scale };

				registry.emplace<CVisual>(tile, srcRect, dstRect, graphics.loadTexture(textureFilename), 0);
				...
				if (collidable)
				{
					std::cout << "Tile(" << j << ", " << i << ") is collidable\n";
				}

				registry.emplace<CSpatial>(tile, collisionRect, collidable);
			}
		}
